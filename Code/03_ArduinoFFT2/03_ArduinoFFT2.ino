/*
 * Credit to Enrique Condés and Ragnar Ranøyen Homb, example from ArduinoFFT library taken and modified
 */
...
// array that contains the conter for all microphones
float maxCounts[microphone_count];
...
// method to flush data from a given array
void flushArray(float arr[],int arrSize) {
  for(int i=0;i<arrSize;i++) {
    arr[i] = 0;
  }
}
...
void determineAngle() {
  ...
  // determine source for each frequency part
  flushArray(maxCounts,samples);
  for(int i=0;i<samples;i++) {
    float arr[] = {Real1[i],Real2[i],Real3[i],Real4[i],Real5[i]};
    int index = findMaximumIndex(arr,microphone_count);
    maxCounts[index]++;
  }
  // determine angle
  angle = calculateAngle(maxCounts,microphone_count);
  long totalTime = micros()-startTime;            // time measurement
  // Print for debugging
